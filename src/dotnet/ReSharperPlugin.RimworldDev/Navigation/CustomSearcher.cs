...
namespace ReSharperPlugin.RimworldDev.Navigation;
  ...
  public class CustomSearcher<TLanguage> : IDomainSpecificSearcher where TLanguage : KnownLanguage
  {
    ...
    {
      foreach (string word in myWordsInText)
      {
        if (myWordIndex.CanContainWord(sourceFile, word))
          return true;
      }
      return false;
    }
    ...
    public bool ProcessElement<TResult>(ITreeNode element, IFindResultConsumer<TResult> consumer)
    {
      ...
      JetHashSet<string> referenceNames = new JetHashSet<string>();
      foreach (IDeclaredElement e2 in myElements)
      {
        myNames.Add(e2.ShortName);
      }
      return (!mySearchForLateBound ? new ReferenceSearchSourceFileProcessor<TResult>(element, myReferenceSearcherParameters, consumer, myElements, myWordsInText, referenceNames) : (NamedThingsSearchSourceFileProcessor) new LateBoundReferenceSourceFileProcessor<TResult>(element, consumer, myElements, myWordsInText, referenceNames)).Run() == FindExecution.Stop;
    }
  }
